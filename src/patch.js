...
LOS.setSkin = function(skin){
  ...
  $$('#losBar .seg button').forEach(function(b){ b.classList.toggle('on', b.dataset.skin === skin); });
};
function guardSkin(){
...
function paintSeg(){
  var s = LOS.skin();
  $$('#losBar .seg button').forEach(function(b){ b.classList.toggle('on', b.dataset.skin === s); });
}
...
var CALC_TABS = [
  ...
  ['sche','Schedule E'], ['va','VA Income'], ['other','Other Income'], ['assets','Assets'],
...
var FIXED_SUBS = {
  ...
  'c:va':      [['calc','VA income'], ['residual','Residual income test'], ['guides','Guidelines']],
...
/* ================================================================== 2b
   APP BAR CLUSTER
   Theme, sync, autosave, the scenario workbench and the hand-off to the
   renovation side. This is everything the left rail used to carry, put
   back where the file's own controls already live.
   ================================================================== */
function buildBar(){
  if ($('#losBar')) return;
  ...
  bar.innerHTML =
      ...
      + '</div>'
    + '<button class="lbtn" data-act="scen">Scenarios</button>'
    + '<button class="lbtn go" data-act="park">Renovation Suite &rarr;</button>'
    + '<span class="state" id="losState"></span>';

  var anchor = document.querySelector('#calc-root .appbar .spacer');
  ...
  bar.addEventListener('click', function(e){
    ...
    if (b.dataset.act === 'scen') return SCEN.open();
    if (b.dataset.act === 'park') return LOS.park(true);
  });

  /* the strip that says the income side is parked, and brings it back */
  var note = document.createElement('div');
  note.id = 'losParkNote'; note.className = 'no-print';
  note.innerHTML = '<span>The income worksheets are parked while you work the renovation file. '
    + 'Everything stays calculated and keeps syncing across.</span>'
    + '<button type="button" data-act="unpark">Bring the income tabs back</button>'
    + '<button type="button" data-act="hide">Hide them entirely</button>';
  var tabbar = document.querySelector('#calc-root nav.tabbar');
  if (tabbar && tabbar.parentNode) tabbar.parentNode.insertBefore(note, tabbar);
  note.addEventListener('click', function(e){
    var b = e.target.closest('button'); if (!b) return;
    if (b.dataset.act === 'unpark') LOS.park(false);
    if (b.dataset.act === 'hide'){
      document.getElementById('calc-root').classList.toggle('los-collapsed');
      b.textContent = document.getElementById('calc-root').classList.contains('los-collapsed')
        ? 'Show the income tabs' : 'Hide them entirely';
    }
  });
  paintSeg();
}

/* One button: hand over to the suite and fade the income side back, so the
   tabs are still one click away rather than gone. */
LOS.park = function(on){
  var root = document.getElementById('calc-root');
  if (!root) return;
  root.classList.toggle('los-parked', !!on);
  if (!on) root.classList.remove('los-collapsed');
  if (on){
    AUTO.commitIfDirty('moving to the renovation suite');
    pushIncome(true);
    var sh = shell(); if (sh && sh.mode !== 'suite') sh.go('suite');
    LOS.tab = 's:' + ((suite() && suite().snapshot.mode) || 'quote');
    setTimeout(function(){ renderSubs(LOS.tab); }, 80);
  } else {
    var sh2 = shell(); if (sh2 && sh2.mode !== 'calc') sh2.go('calc');
  }
  paintNav();
};

/* Route to a tab. The pending autosave is committed here, and only here:
   the brief was to write on leaving a tab, not on every keystroke. */
LOS.go = function(key){
  ...
  if (side === 'c'){
    ...
    /* leaving one of the moved panels has to put the suite's own columns
       back, or the screen switches underneath a hidden view */
    if (window.LOANSUITE && LOANSUITE.goSuite){ LOANSUITE.goSuite(id); }
    else { var st = suite(); if (st) { try { st.setMode(id); } catch(e){} } }
  }
  paintNav();
  setTimeout(function(){ renderSubs(key); }, 60);
};

/* The native tab bars are the navigation now; this only mirrors state. */
function paintNav(){ paintFigures(); }

/* ------------------------------------------------- live figures in tabs */
function paintFigures(){
  try {
    var st = suite();
    if (st){
      var el = $('#losState');
      if (el && !el.dataset.sticky){
        var o = st.outputs;
        el.innerHTML = '<span class="dotlive ' + (LOS.syncOn() ? (LOS._stale ? 'stale' : '') : 'off') + '"></span>'
          + 'Suite loan ' + usd(pick(o,['loan.maximumBaseLoan']));
      }
    }
  } catch(e){}
}
...
/* ================================================================== 3
   ...
   Derived from what the page actually renders, so a screen that gains a
   section gains a subtab without anything here changing.
   ================================================================== */
...
function sectionsIn(host){
  ...
  for (var i = 0; i < candidates.length; i++){
    ...
    if (h.closest && h.closest('.rail, .tabs, .tabbar, .toolbar, .topbar, .legend')) continue;
...
function hostFor(key){
  ...
  return (cm && cm.firstElementChild) || sr;
}
...
function applySub(key, sub){
  ...
  if (fixed){
    ...
    if (key === 'c:va'){
      var map = { calc:'#vaRecords', residual:'#vaResidual', guides:'#vaGuides' };
      Object.keys(map).forEach(function(k){
        var el = $(map[k]); if (el) el.classList.toggle('los-hidden', sub !== k);
      });
      var tot = document.querySelector('#vaBody > .card');
      if (tot) tot.classList.remove('los-hidden');
    }
    if (key === 'c:summary'){
      var body = $('#summaryBody'), gh = $('#guideHead'), gb = $('#guideBody');
      if (body) body.classList.toggle('los-hidden', sub !== 'sum');
...
function applyFilter(key, sub, quiet){
  ...
  /* The suite rebuilds its screen on every store change, so a section list
     captured a moment ago can point at elements that are no longer in the
     document. Re-derive rather than silently filtering nothing. */
  if (!secs || !secs.length || secs.some(function(s){ return !s.el || !s.el.isConnected; })){
    secs = sectionsIn(hostFor(key));
    LOS._secs[key] = secs;
  }
...
/* patch-va.js wraps switchTab outermost and short-circuits for its own
   panel, so it needs a way back in to draw the strip. */
LOS.refreshSubs = function(key){ LOS.tab = key; renderSubs(key); };
...
function renderStrip(key){
  ...
  /* Only the active tab's strip may be on screen — one left behind from a
     previous tab is both confusing and clickable. */
  /* Strips are rebuilt per tab, so a leftover one is removed rather than
     hidden — hidden ones are still in the document and still clickable. */
  $$('.los-strip').forEach(function(el){ if (el !== existing) el.remove(); });
  if (!existing){
...
function state(msg){
  ...
  state._t = setTimeout(function(){ delete el.dataset.sticky; }, 6000);
}
function pushIncome(force){
  ...
  try {
    ...
    if (LOS._sawFirstSync) state((t ? usd(t.income,2) + ' / mo synced' : 'Income synced'));
    LOS._sawFirstSync = true;
